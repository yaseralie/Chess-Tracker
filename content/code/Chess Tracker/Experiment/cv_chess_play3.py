            ...
            if ref_frame is None:
                ...
                if len(detected) == 2:
                    ...
                    if piece_a and not piece_b:
                        ...
                        # --- Tambahan heuristik baru ---
                        # Coba pilih salah satu yang memiliki bidak sebelumnya
                        if prev_board.piece_at(chess.parse_square(a)):
                            from_sq, to_sq = a, b
                        elif prev_board.piece_at(chess.parse_square(b)):
                            from_sq, to_sq = b, a
                        else:
                            # fallback lama (rank-based)
                    ...
                    # hanya satu kotak berubah
                    to_sq = list(detected)[0]
                    piece_now = board.piece_at(chess.parse_square(to_sq))

                    if piece_now:
                        # kemungkinan capture di tempat -> cari semua move yang berakhir di sini
                        candidates = [m for m in board.legal_moves if m.uci()[2:] == to_sq]
                        if candidates:
                            mv_best = candidates[0]
                            from_sq = mv_best.uci()[:2]
                            to_sq = mv_best.uci()[2:]
                    else:
                        # kemungkinan bidak bergerak dari rank berdekatan (contoh d2 → d3)
                        rank = int(to_sq[1])
                        file = to_sq[0]
                        adj = None
                        ...
                        if board.turn == chess.WHITE and rank > 1:
                            adj = f"{file}{rank-1}"
                        elif board.turn == chess.BLACK and rank < 8:
                            adj = f"{file}{rank+1}"
